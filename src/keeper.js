...
async function loadPbxs(pbtid) {
  ...
  for (let i = 0; i < atArr.length; i++) {
    ...
    const depAddr = window.ChiaUtils.puzzle_hash_to_address(
      String(xAddress[0]),
      winfo.prefix
    );
    const withAddr = window.ChiaUtils.puzzle_hash_to_address(
      String(xAddress[1]),
      winfo.prefix
    );
    const addrInfo = {
...
async function loadMarketInfo(id) {
  const sinfo = await bsc.ctrs.pbmarket.getSaleInfo(
...
async function initMyList(bsc, commit) {
  ...
  for (let i = 0; i < cnt; i++) {
    const idx = (
      await bsc.ctrs.pbt.tokenOfOwnerByIndex(bsc.addr, i)
    ).toString();
    myList[idx] = {
      loading: true,
      id: idx,
...
async function initMarketList(bsc, commit) {
  ...
  for (let i = 0; i < cnt; i++) {
    const idx = (
      await bsc.ctrs.pbt.tokenOfOwnerByIndex(bsc.ctrs.pbmarket.address, i)
    ).toString();
    ids[i] = idx;
    setMarketItem(
      idx,
...
async function startKeeper(_bsc, commit) {
  ...
  if (bsc.ctrs.pbpuzzlehash.filters.WithdrawPuzzleHashChanged) {
    bsc.ctrs.pbpuzzlehash.on(
      bsc.ctrs.pbpuzzlehash.filters.WithdrawPuzzleHashChanged,
      async function (evt) {
  ...
  if (bsc.ctrs.pbpuzzlehash.filters.DepositPuzzleHashChanged) {
    bsc.ctrs.pbpuzzlehash.on(
      bsc.ctrs.pbpuzzlehash.filters.DepositPuzzleHashChanged,
      async function (evt) {
  ...
  if (bsc.ctrs.pbmarket.filters.OnSale) {
    bsc.ctrs.pbmarket.on(
      bsc.ctrs.pbmarket.filters.OnSale,
      async function (evt) {
        if (evt.event == "OnSale") {
          console.log("keeper", evt.event, "info", evt);
